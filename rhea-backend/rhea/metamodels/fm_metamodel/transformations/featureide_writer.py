...
class FeatureIDEWriter(ModelToText):
    ...
    CTC_TYPES = {ASTOperation.NOT: 'NotTerm',
                 ...
                 ASTOperation.XOR: 'disj',
                 ASTOperation.IMPLIES: 'ImpliesTerm',
                 ASTOperation.REQUIRES: 'Impl',
    ...
    def transform(self) -> str:
        xml_str = _to_featureidexml(self.source_model)
        if self.path is not None:
            with open(self.path, 'w', encoding='utf8') as file:
                file.write(xml_str, file, indent=4)
        return xml_str


def _to_featureidexml(feature_model: FeatureModel) -> dict[str, Any]:
    # for relation in feature_model.root.get_relations():
