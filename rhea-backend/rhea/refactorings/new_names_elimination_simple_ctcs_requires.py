...
class NewNamesEliminationSimpleConstraintsRequires(FMRefactoring):
    ...
    def transform(model: FeatureModel, instance: Constraint) -> FeatureModel:
        ...
        for feature in model.get_features():
            if right_feature_ctc_plus==feature:
                model = transform_each_feature(model, feature, model_plus)
        
        for feature in model.get_features():
            if right_feature_ctc_less==feature:
                model = transform_each_feature(model, feature, instance)
        
        for feature in model.get_features():
            if left_feature_ctc_less==feature:
                model = transform_each_feature(model, feature, instance)
...
def transform_each_feature(model: FeatureModel) -> FeatureModel:
