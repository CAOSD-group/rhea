...
//	}
	...
	private static String createEncoding(List<String> expressions) {
		...
		for (int i=0; i<expressions.size(); i++) {
			orPart.append(expressions.get(i));
			if (i<expressions.size()-1)
				orPart.append(" | ");
		}
		orPart.append(")");
		result.append(orPart.toString());
		

		StringBuilder andPart = new StringBuilder();

		result.append(andPart.toString());	
		for (int i=0; i<expressions.size(); i++) {
			for (int j=i+1; j<expressions.size(); j++) {
					andPart.append(" & !(");
					andPart.append(expressions.get(i));
					andPart.append(" & ");
					andPart.append(expressions.get(j));
					andPart.append(")");
			}
		}
