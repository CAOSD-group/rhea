...
public class JavaNumericalFeatureRefactoring extends Refactoring{

	public JavaNumericalFeatureRefactoring(FeatureModel featureModel) {
		super(featureModel);
		classPath = "rhea.metamodels.NumericalFMs.NumericalFeature";
	}
	...
	public List<TransformationInformation> refactor(int times, String output) {
		...
		JavaNumericalFeatureRule nfr = new JavaNumericalFeatureRule(EcoreUtil.copy(fm), classPath);
		...
		for(int i = 0; i < times ; i++)
		{
			...
			tf.setNumberOfFeaturesTypeBefore(nfr.getMatchingFeatures().size());
			...
			//Do the refactoring
			if(nfr.executeRefactoring()) System.out.println("Transformation applied sucessfully");
			
			//Get the information after the refactoring
			tf.setTimeAfter(System.nanoTime()/1e9);
			tf.setnFeaturesAfter(f.getFeatures().size());
			tf.setNumberOfFeaturesTypeAfter(nfr.getMatchingFeatures().size());
		...
		Utils.serialize(g.fm2text(nfr.getFeatureModel()), outputFile);
